...
const Signup = () => {
  ...
  return (
    ...
    >
      <div className="relative w-[400px] h-[600px] bg-white/80 backdrop-blur-lg rounded-3xl shadow-xl flex flex-col items-center p-8">
        <div className="absolute top-0 left-0 w-full bg-[#66bb6a] p-4 rounded-t-3xl text-center">
          <img
            src={NITLogo}
            alt="NIT Logo"
        ...
        <h2 className="mt-20 text-2xl font-bold text-[#1b5e20] shadow-md p-2 text-center">
          Library User Registration Portal
        </h2>
        {error && <p className="text-red-500 text-sm mt-2">{error}</p>}
        {success && <p className="text-green-500 text-sm mt-2">{success}</p>}
        <form onSubmit={handleSignup} className="w-full flex flex-col items-center mt-4">
          <div className="w-full mb-3">
            <input
              type="text"
              className="w-full p-4 border border-[#66bb6a] rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1b5e20] transition-all duration-300"
              placeholder="Full Name"
              value={name}
              onChange={(e) => setName(e.target.value)}
              required
          ...
          <div className="w-full mb-4">
            <button
              type="submit"
              className="w-full py-3 rounded-xl bg-[#66bb6a] text-white font-semibold hover:bg-[#43a047] transition-all duration-300"
            >
              Sign Up
            </button>
          </div>
